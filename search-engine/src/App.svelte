...
<script>
	...
	function handleKeyDown(event, result) {
        ...
        event.preventDefault();
        console.log("Modal key down event for:", result); 
        showModal(result);
        }
    }
</script>
<main>
    ...
    <form id="search-box">
        ...
        <button  on:click={formSubmitted} id="search-btn" class="search-btn" >
        ...
        <div class="results-section">
            ...
            <div id="results-container">
                ...
                {#each currentResults as result}
					<div class="result-item" on:click={() => showModal(result)} on:keydown={(event) => handleKeyDown(event, result)} data-id={result.id}>
						...
						<div id="modal-message" style="text-align: left;">
                            ...
                            <div class="similar-docs">
                              {#each findSimilarDocuments(selectedResult) as doc}
                                <div class="clickable" on:click={() => showSimilarModal(doc)} on:keydown={(event) => handleKeyDown(event, doc)} data-id={doc.id}>- {doc.title}</div>
                              {/each}
                            </div>
                          {:else}
